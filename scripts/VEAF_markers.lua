...
veafCas.RadioMenuPrefix = "VEAF "
veafCas.CasRadioMenuPrefix = veafCas.RadioMenuPrefix .. "- CAS"
veafCas.HelpRadioMenuPrefix = veafCas.RadioMenuPrefix .. "- HELP (" .. veafCas.version .. ")"
-------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function veafCas.generateCasMission(spawnSpot, size, defense, armor, spacing, skill, disperseOnAttack)
    ...
    -- add the main radio menu
    veafCas._casPath = missionCommands.addSubMenu(veafCas.CasRadioMenuPrefix)

    -- add radio menu information messages
    veafCas._targetInfoPath = missionCommands.addSubMenu("Target information", veafCas._casPath)
...
function veafCas.smokeCasTargetGroup()
	...
	missionCommands.removeItem({veafCas.CasRadioMenuPrefix, 'Target markers', 'Request smoke on target area'})
...
function veafCas.smokeReset()
	missionCommands.removeItem({veafCas.CasRadioMenuPrefix, 'Target markers', 'Target is marked with red smoke'})
	missionCommands.addCommand('Request smoke on target area', veafCas._targetMarkersPath, veafCas.smokeCasTargetGroup)
...
function veafCas.flareCasTargetGroup()
	...
	missionCommands.removeItem({veafCas.CasRadioMenuPrefix, 'Target markers', 'Request illumination flare over target area'})
...
function veafCas.flareReset()
	missionCommands.removeItem({veafCas.CasRadioMenuPrefix, 'Target markers', 'Target area is marked with illumination flare'})
    missionCommands.addCommand('Request illumination flare over target area', veafCas._targetMarkersPath, veafCas.flareCasTargetGroup)
...
function veafCas.cleanupAfterMission()
    ...
    missionCommands.removeItem({veafCas.CasRadioMenuPrefix})
end

--- Build the initial radio menu
function veafCas.buildRadioMenu()
    local _infoPath = missionCommands.addSubMenu(veafCas.HelpRadioMenuPrefix)
    missionCommands.addCommand('create a marker and use one of these commands in the text :', _infoPath, veafCas.emptyFunction)
    local _casInfoPath = missionCommands.addSubMenu('"veaf do cas" creates a default CAS target group - see options', _infoPath)
    missionCommands.addCommand('"defense 0" completely disables air defenses', _casInfoPath, veafCas.emptyFunction)
    missionCommands.addCommand('"defense [1-5]" specifies air defense cover (1 = light, 5 = heavy)', _casInfoPath, veafCas.emptyFunction)
    missionCommands.addCommand('"size [1-5]" changes the group size (1 = small, 5 = huge)', _casInfoPath, veafCas.emptyFunction)
    missionCommands.addCommand('"armor [1-5]" specifies armor presence (1 = light, 5 = heavy)', _casInfoPath, veafCas.emptyFunction)
    missionCommands.addCommand('"spacing [1-5]" changes the groups spacing (1 = dense, 3 = default, 5 = sparse)', _casInfoPath, veafCas.emptyFunction)
    local _spawnInfoPath = missionCommands.addSubMenu('"veaf do spawn" spawns a target vehicle/ship - see options', _infoPath)
